...
local kvpKeywords = {
    position = "custom-spawnmanager:Position"
}
...
-- load KVP Data
local function loadKVPdata(keyword,type)
    local reference = {
        float = GetExternalKvpFloat,
        int = GetResourceKvpInt,
        str = GetResourceKvpString
    }
    if reference[type] ~= nil then
        return reference[type](keyword)
    end
end
...
function spawnPlayer(spawnIdx, cb)
    ...
    Citizen.CreateThread(function()
        ...
        local prevCoords = json.decode(loadKVPdata(kvpKeywords.position,"str"))

        if prevCoords ~= nil and type(prevCoords) == "table" then
            spawn.x = prevCoords.x
            spawn.y = prevCoords.y
            spawn.z = prevCoords.z
        end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if playerPed and playerPed ~= -1 then
            ...
            if IsEntityDead(playerPed) --[[ and not exports.utils.IsKnockedOut ]] then
                if not diedAt --[[ and not HasPedBeenDamagedByWeapon(PlayerPedId(), GetHashKey("WEAPON_UNARMED"), 0) ]] then
                    diedAt = GetGameTimer()
                    TriggerEvent('playerDied')
                elseif not diedAt then
                    -- TriggerEvent("knockedOut")
                end
            else
                diedAt = nil
            end
...
Citizen.CreateThread(function ()
    while true do
        ...
        if coords and coords.z ~= 0 and coords.x ~= 0 and coords.y ~= 0 then
            SetResourceKvp(kvpKeywords.position,json.encode(coords))
        end
        Citizen.Wait(5000)
    end
end)
